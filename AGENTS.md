...
12. GitHub Pages is currently public by explicit user decision. A post may be public only when every included user and GPT message has been verified against the source conversation, assigned a source message ID and ordinal, and hashed. Never publish or commit summaries, reconstructions, excerpts, hidden reasoning, tool output, or unavailable replies as substitutes for visible source messages.
...
9. Validate, build, commit atomically, and report what was added, corrected, or held for source verification.
